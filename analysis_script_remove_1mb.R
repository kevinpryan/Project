...
# Set up biomaRt for getting all genes within 1mb of each variant so we can exclude them
...
for (i in 1:nrow(tf.dbdp.uniq.indorothea)){
	...
	range.start <- as.character(max(1,var.position - 1000000))
	range.end <- as.character(var.position + 1000000)
	values <- list(chromosome = var.chromosome, start=range.start,end=range.end)
	...
	tf.targets.interest.filter.range <- subset(tf.targets.interest, !(tf.targets.interest %in% all.genes$hgnc_symbol))
	intersect.tf.targets.frac.rank <- intersect(tf.targets.interest.filter.range, rownames(frac.rank.samples.matched))
