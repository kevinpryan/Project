...
targets_interest <- c("JAG1", "SH3YL1")
...
ggar_obj <- ggarrange(plotlist = myplots, nrow = 1, ncol = 2, common.legend = TRUE)#, legend = "bottom")
ggar_obj_annotated <- annotate_figure(ggar_obj, left = textGrob("Target gene expression", rot = 90, vjust = 1, gp = gpar(cex = 1.6)), bottom = textGrob("IRF6 expression (Whole Blood) ", gp = gpar(cex = 1.6)))
