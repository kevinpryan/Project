...
# To run for example the default approach using the fractional rank data, filtering approach and all covariates - Rscript enrichment_analysis_filtering_interaction.R default frac_rank pass all_covariates filtered_tf
...
# Edit line 336 to write to file of interest 
# The purpose of this script is to do our filtering and see if the targets are enriched for interaction-eQTLs vs non-targets
# Load in packages
library(stringr)
library(biomaRt)
library(vroom)
...
for (i in 1:nrow(tf.dbdp.uniq.indorothea)){
	...
	main.effect.pvals.outersect <- c()
	interaction.model.pvals.outersect <- c()
	...
	if (args[2] == "whole_blood_norm"){
		...
		if (linear.model.pval < 0.05 & sign.linear.model == sign.mor){
                        if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
                                interaction.model <- lm(tab[,j] ~ expression.data.tf.vec + expression.data.tf.vec:tab$tf.genotypes + as.matrix(cov_t))
                         } else if (args[4] == "no_covariates"){
                                interaction.model <- lm(tab_rm_na[,j] ~ expression.data.tf.vec + expression.data.tf.vec:tab_rm_na$tf.genotypes)

                         }
			interaction.model.pval <- coef(summary(interaction.model))["expression.data.tf.vec:tab$tf.genotypes", "Pr(>|t|)"]
	...
	for (l in 2:ncol(tab_outersect_rm_na)){
		if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
                                interaction.model.outersect <- lm(tab_outersect_rm_na[,l] ~ expression.data.tf.vec + expression.data.tf.vec:tab_outersect_rm_na$tf.genotypes + as.matrix(cov_t), na.action = na.omit)
				interaction.model.outersect.pval <- coef(summary(interaction.model.outersect))["expression.data.tf.vec:tab_outersect_rm_na$tf.genotypes", "Pr(>|t|)"]
				main.effect.pval <- coef(summary(interaction.model))["expression.data.tf.vec", "Pr(>|t|)"]
				main.effect.pvals.outersect <- c(main.effect.pvals, main.effect.pval)
				interaction.model.pvals.outersect <- c(interaction.model.pvals.outersect, interaction.model.outersect.pval)
                 } else if (args[4] == "no_covariates"){
				interaction.model.outersect <- lm(tab_outersect_rm_na[,l] ~ expression.data.tf.vec + expression.data.tf.vec:tab_outersect_rm_na$tf.genotypes, na.action = na.omit)
                                interaction.model.outersect.pval <- coef(summary(interaction.model.outersect))["expression.data.tf.vec:tab_outersect_rm_na$tf.genotypes", "Pr(>|t|)"]
                                main.effect.pval.outersect <- coef(summary(interaction.model.outersect))["expression.data.tf.vec", "Pr(>|t|)"]
                                main.effect.pvals.outersect <- c(main.effect.pvals.outersect, main.effect.pval.outersect)
                                interaction.model.pvals.outersect <- c(interaction.model.pvals.outersect, interaction.model.outersect.pval)
                 }
	}
	main.effect.pvals.remove.na <- na.omit(main.effect.pvals)
	main.effect.pvals.outersect.remove.na <- na.omit(main.effect.pvals.outersect)
	interaction.model.pvals.remove.na <- na.omit(interaction.model.pvals)
	interaction.model.pvals.outersect.remove.na <- na.omit(interaction.model.pvals.outersect)
	interaction_targets <- length(which((main.effect.pvals.remove.na < 0.05 & interaction.model.pvals.remove.na < 0.05) == TRUE))
	interaction_nontargets <- length(which((main.effect.pvals.outersect.remove.na < 0.05 & interaction.model.pvals.outersect.remove.na < 0.05) == TRUE))
	noninteraction_targets <- length(which((main.effect.pvals.remove.na < 0.05 & interaction.model.pvals.remove.na < 0.05) == FALSE))
	noninteraction_nontargets <- length(which((main.effect.pvals.outersect.remove.na < 0.05 & interaction.model.pvals.outersect.remove.na < 0.05) == FALSE))
	print(paste("interaction_targets:", interaction_targets))
	print(paste("interaction_nontargets:", interaction_nontargets))
	print(paste("noninteraction_targets:", noninteraction_targets))
	print(paste("noninteraction_nontargets:", noninteraction_nontargets))
   	contingency_tab <- array(c(interaction_targets, interaction_nontargets, noninteraction_targets, noninteraction_nontargets), c(2,2))
	...
	no.of.targets.after.filtering <- c(no.of.targets.after.filtering, length(main.effect.pvals.remove.na))
  }
  ...
  write.table(results.sorted, file = paste("/data/kryan/project/gtex/outputs/trans_eqtl_results/", args[2], "/all_targets/", args[4], "/", args[5], "/enrichment_", args[2], "_", args[4], "_interaction_filtered_norsq_AF1.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
 end.time <- Sys.time()
