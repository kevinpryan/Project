...
# This script carried out interaction-eQTL analysis on a single tissue (whole blood or fractional rank data)
# Script was developed with R version 4.1.0
# Files required: genotype data GTEx extracted according to instructions (see README), dbdp_nssnp_barrera_with_GTEx_varid_cadd.txt, covariates - either Whole_Blood.v8.covariates.txt from GTEx or covariates_frac_rank_extracted_exp_and_geno_rownames_altered.txt - generated using extract_covariates.R
# gtex.psam - GTEx sample info, dorothea_hs_with_target_info.txt. Change file paths as necessary.
# To run - Rscript 
# Load in packages
library(stringr)
library(biomaRt)
...
print(paste("Arg 1:",args[1], "arg 2:",args[2],"arg 3:", args[3], "arg 4:", args[4], "arg 5:", args[5]))
# Point to file names
genotypes <- "/data/kryan/project/gtex/genotype_data_dbdp.vcf"
allele.frequency.desired <- 1.0e-02
barrera.variants <- "/data/kryan/project/gtex/analysis/dbdp_nssnp_barrera_with_GTEx_varid_cadd.txt"
...
# Reduce down covariate file if specified - do nothing otherwise
...
print("just before if else statement")
if(args[1] == "default"){
...
#for (i in c(1:2)){
	...
	for (j in 2:ncol(tab_rm_na)){
		#print(paste("fitting linear model var:", var, "tf:",tf.interest,  "target:", intersect.tf.targets.expression.data[j-1] ))
		#print(paste("j:", j, "ncol(tab_rm_na):", ncol(tab_rm_na)))
		...
		#print(paste("linear model pval:", linear.model.pval))
		#print(paste("sign.linear.model:", sign.linear.model))
		#print(paste("mor sign", sign.mor))
		if (linear.model.pval < 0.05 & sign.linear.model == sign.mor){
			#print(paste("fitting interaction model var:", var, "target:", intersect.tf.targets.expression.data[j-1] ))
			#interaction.model.pval <- summary(lm(tab_rm_na[,j] ~ expression.data.tf.vec + expression.data.tf.vec:tab_rm_na$tf.genotypes + as.matrix(cov_t)))$coefficient[71,4]
			if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
	...
	#print(interaction.model.pvals)
	#print(summary(lm(tab_rm_na[,3] ~ expression.data.tf.vec + expression.data.tf.vec:tab_rm_na$tf.genotypes)))
	#for (j in 2:ncol(tab)){
 	#	 linear.model <- lm(tab[,j] ~ tab$tf.genotypes + as.matrix(cov_t))
...
#	results <- cbind(rep(var, (ncol(tab_rm_na) - 1)), 
#	                 rep(tf.interest, (ncol(tab_rm_na) - 1)), intersect.tf.targets.expression.data, df.targets.intersect$tf.targets.names, df.targets.intersect$chrom.target.range, interaction.model.pvals, df.targets.intersect$mor.targets, df.targets.intersect$confidence.targets)
  ...
  #colnames(df.out) <- c("Variant_GTEx_ID", "TF_containing_variant", "interaction_p_value")
 ...
 # print(FDR.adjusted.results, row.names = FALSE)
  ...
  print(FDR.adjusted.results[FDR.adjusted.results$FDR.adjusted.pvals < 0.05,]) 
  FDR.adjusted.results.sorted <- FDR.adjusted.results[order(FDR.adjusted.results$interaction.FDR.adjusted.pvals),]
  print("full results")
  print(FDR.adjusted.results.sorted[1:20,])
  write.table(FDR.adjusted.results.sorted, file = paste("/data/kryan/project/gtex/outputs/trans_eqtl_results/", args[2], "/all_targets/", args[4], "/", args[5], "/filter_sign/",  args[2], "_", args[4], "_", "interaction_FDR_adjusted_results_AF1_filter_no_rsquared_add_main_effect.txt", sep = ""), quote = FALSE, sep = "\t", row.names = FALSE)
       ...
       if (args[5] == "filtered_tf"){
		...
		interaction.model.pval.upreg <- NA
				...
				interaction.model.pval.upreg <- NA
                        ...
                        if (ncol(tab_upreg) == 2){
                                print(paste("ncol tab_upreg:", ncol(tab_upreg) ))
                                print("entering tab_upreg = 2")
                                if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
                                ...
                                if (linear.model.pval < 0.05 & sign.linear.model.upreg == 1){
                                 ...
                                 #p.values.upreg <- c(p.values.upreg, summary(linear.model.eqtl)$coefficients[2,4])
                                 #cof.upreg <- c(cof.upreg,summary(linear.model.eqtl)$coefficients[2,1])
				 ...
				 interaction.model.pval.upreg <- coef(summary(interaction.model.upreg))["expression.data.tf.vec:tab_upreg$tf.genotypes", "Pr(>|t|)"]
                                 #adjusted_rsquared <- summary(linear.model.eqtl)$adj.r.squared
                                } else {
                                        #p.values.upreg <- c(p.values, NA)
                                        #cof.upreg <- c(cof.upreg,NA)
                                        p.values.upreg <- NA
                                        cof.upreg <- NA
					...
					interaction.model.pval.upreg <- NA
                                ...
                                print(paste("ncol tab_upreg:", ncol(tab_upreg) ))
                                print("entering else statement, should be more than 2 columns in tab_upreg")
                                for (j in 2:ncol(tab_upreg)) {
                                        ...
                                        sign.linear.model.upreg <- sign(coef(summary(linear.model.upreg))["expression.data.tf.vec", "Estimate"])
                                        adjusted.rsquared.linear.model <- summary(linear.model.upreg)$adj.r.squared
                                        #print(paste("sign.linear.model:", sign.linear.model.upreg))
                                        #if (linear.model.pval >= 0.05 | sign.linear.model.upreg != 1 | adjusted.rsquared.linear.model <= 0.3){
                                        if (linear.model.pval >= 0.05 | sign.linear.model.upreg != 1){
...
if (length(tf.targets.downreg) == 0){
                                #cof.downreg <- c(cof.downreg,NA)
                                #p.values.downreg <- c(cof.downreg,NA)
                                cof.downreg <- NA
                                p.values.downreg <- NA
				interaction.model.pval.downreg <- NA
                                print(paste("No downregulated genes in variant:", i))
                } else {
			print("more than 0 downreg targets")
                        ...
                        if (ncol(tab_downreg) == 2){
                                ...
                                #sign.mor <- sign(mor.targets[j-1])
                                print(paste("linear model pval:", linear.model.pval))
                                #if (linear.model.pval < 0.05 & sign.linear.model.downreg == -1 & adjusted.rsquared.linear.model > 0.3){
                                if (linear.model.pval < 0.05 & sign.linear.model.downreg == -1){
                                        ...
                                        print(paste("tab_rank_downreg_mean, 2 cols:", tab_rank_downreg_mean))                  
                                if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
                                 ...
                                 #p.values.downreg <- c(p.values.downreg, summary(linear.model.eqtl)$coefficients[2,4])
                                 #cof.downreg <- c(cof.downreg,summary(linear.model.eqtl)$coefficients[2,1])
				 ...
				 interaction.model.pval.downreg <- coef(summary(interaction.model.downreg))["expression.data.tf.vec:tab_downreg$tf.genotypes", "Pr(>|t|)"]
                                        ...
                                        #p.values.downreg <- c(p.values.downreg, NA)
                                        #cof.downreg <- c(cof.downreg,NA)
                                        p.values.downreg <- NA
                                        cof.downreg <- NA
					...
					interaction.model.pval.downreg <- NA
                                ...
                                for (j in 2:ncol(tab_downreg)) {
                                        ...
                                        #sign.mor <- sign(mor.targets[j-1])
                                        #print(paste("linear model pval:", linear.model.pval))
                                        #print(paste("sign.linear.model:", sign.linear.model.downreg))
                                        #if (linear.model.pval >= 0.05 | sign.linear.model.downreg != -1 | adjusted.rsquared.linear.model <= 0.3){
                                        if (linear.model.pval >= 0.05 | sign.linear.model.downreg != -1){
 ...
 print("trying to combine dataframe of tab_downreg$tfgenotypes and tab_rank_downreg_mean")
        ...
        } else if (args[5] == "no_filter_tf"){
		print("entering no filter method")
		if (args[2] == "whole_blood_norm"){
                        expression.data.tf <- expression.data.samples.matched[tf.interest.ensembl,]
                } else if (args[2] == "frac_rank" | args[2] == "whole_blood_tpm") {
                        expression.data.tf <- expression.data.samples.matched[tf.interest,]
                ...
                print(paste("No expression data for tf, skipping...", tf.interest))
                #cof.upreg <- c(cof.upreg,NA)
                #p.values.upreg <- c(p.values.upreg,NA)
                cof.upreg <- NA
                p.values.upreg <- NA
                interaction.model.pval.upreg <- NA
                next
		...
		interaction.model.pval.upreg <- NA
                ...
                if (args[4] == "all_covariates" | args[4] == "reduced_covariates"){
                	interaction.model.upreg <- lm(tab_rank_upreg_mean_genotypes$tab_rank_upreg_mean ~ expression.data.tf.vec + expression.data.tf.vec:tab_rank_upreg_mean_genotypes$tf.genotypes + as.matrix(cov_t))
                        ...
                        interaction.model.upreg <- lm(tab_rank_upreg_mean_genotypes$tab_rank_upreg_mean ~ expression.data.tf.vec + expression.data.tf.vec:tab_rank_upreg_mean_genotypes$tf.genotypes)
		}
                cof.upreg <- c(cof.upreg,summary(linear_model_upreg)$coefficients[2,1])
                p.values.upreg <- c(p.values.upreg, summary(linear_model_upreg)$coefficients[2,4])
...
#results.upreg <- cbind.data.frame(var, tf.interest, p.values.upreg, cof.upreg)
                ...
                print(paste("df.out.upreg:", i))
                print(df.out.upreg)
                print(paste("df.out.downreg:", i))
                print(df.out.downreg)
...
#results.upreg <- cbind.data.frame(var, tf.interest, p.values.upreg, cof.upreg)
             #   results.upreg$mor <- rep(1, nrow(results.upreg))
              #  results.downreg$mor <- rep(-1, nrow(results.downreg))
               # if (ncol(results.upreg) == ncol(results.downreg)){
                #        print("1")
                 #       colnames(results.upreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "main_effect_p_value", "coefficient", "mor", "interaction_p_value")
                  #      colnames(results.downreg) <- colnames(results.upreg)
                   #     df.out.upreg <- rbind.data.frame(df.out.upreg,results.upreg)
                     #   df.out.downreg <- rbind.data.frame(df.out.downreg, results.downreg)
               # } el#se if (ncol(results.upreg) < ncol(results.downreg)){
                #        print("2")
                 #       colnames(results.downreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "main_effect_p_value", "coefficient", "mor", "interaction_p_value")
                  #      df.out.downreg <- rbind.data.frame(df.out.downreg, results.downreg)
               # } else if (ncol(results.upreg) > ncol(results.downreg)){
                #        print("3")
                 #       colnames(results.upreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "main_effect_p_value", "coefficient", "mor", "interaction_p_value")
		...
		#print(df.out.downreg)
            #}
        #}
        #colnames(df.out.upreg) <-  c("Variant_GTEx_ID", "TF_containing_variant", "p_value", "coefficient")
        #colnames(df.out.downreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "p_value", "coefficient")
        #df.out.upreg$mor <- rep(1, nrow(df.out.upreg))
        #colnames(df.out.upreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "p_value", "coefficient", "mor")
        #df.out.downreg$mor <- rep(-1, nrow(df.out.downreg))   
        #colnames(df.out.downreg) <- c("Variant_GTEx_ID", "TF_containing_variant", "p_value", "coefficient", "mor")
        print(colnames(df.out.upreg))
        print(colnames(df.out.downreg))
...
df.out.na.remove$FDR.adjusted.pvals <- p.adjust(as.numeric(df.out.na.remove$"main_effect_p_value"), method = "BH")
	...
	write.table(results.sorted, file = paste("/data/kryan/project/gtex/outputs/trans_eqtl_results/", args[2], "/avg_targets/", args[4], "/", args[5], "/filter_sign/",  args[2], "_", args[4], "_", "interaction_FDR_adjusted_results_AF1_filter_no_rsquared_add_main_effect.txt", sep = ""), quote = FALSE, sep = "\t", row.names = FALSE)
